...
gsc.GeoComment = function(opts){
	...
	var _container=null;
	var _maxFeaturesNumber=(!opts.maxFeaturesNumber)?50:opts.maxFeaturesNumber;
	var _layer=null;
	var _url=null;
...
(function() {
  ...
  var _send=function(){	
	...
	params.push({"key":"layer","value":controls[0].value.substring(0,controls[0].value.lastIndexOf("."))});	
	//make service call
	_serviceCall(params);
  };
  ...
  var _clearAlertMesage=function(){
	...
	for(var i=0;i<controls.length;i++)
	{	
		controls[i].parentElement.className= controls[i].parentElement.className.replace(" has-error","");	
	}
  };
};
